...
// Local stubs for Taskade logger types to remove dependency on private package @taskade/parade-shared
...
function ErrorFallback({
  ...
  // (the lifecycle logger is null). Mirrors base-template-v2.
...
export function GenesisRoot({ children }: { children: React.ReactNode }) {
  return (
    <ReactErrorBoundary
      ...
      onError={(error, info) => {
        // Only log to console in preview/dev mode — the lifecycle logger is
        // only injected by Director Preview, never in the published app.
        if (getGenesisAppLifecycleLogger() != null) {
