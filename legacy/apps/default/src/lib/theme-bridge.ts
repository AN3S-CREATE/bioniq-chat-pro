...
export function setupThemeBridge() {
  window.addEventListener('message', (event) => {
    ...
    // Security: Validate the origin of the parent frame to prevent Clickjacking/XSS
    const origin = event.origin;
    const isTrustedOrigin = 
      origin === 'https://www.taskade.com' ||
      origin === 'https://taskade.com' ||
      /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(origin);
      
    if (!isTrustedOrigin) {
      // Only surface origin warnings in development — avoid leaking origin info to
      // browser consoles in production where non-Taskade iframes may trigger this.
      if (typeof process !== 'undefined' && process.env?.NODE_ENV !== 'production') {
        console.warn(`[ThemeBridge] Ignored message from untrusted origin: ${origin}`);
      }
      return;
    }
    ...
    if (event.data?.type === 'TASKADE_THEME_UPDATE') {
      ...
      // Reply only to the trusted origin that sent the update
      window.parent.postMessage({ type: 'TASKADE_THEME_APPLIED' }, origin);
      return;
    }
